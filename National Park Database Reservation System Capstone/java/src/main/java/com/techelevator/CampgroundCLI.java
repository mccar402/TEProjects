...
public class CampgroundCLI {
	...
	private String[] campgroundNamesArray;
	private Scanner userInput;
	private List<Site> siteList;
	private List<Reservation> reservationList;
//	LocalDate arrival = checkArrivalDate();
//	LocalDate departure = checkDepartureDate();
	...
	public void displayReservationMenu() {
		...
		menu.getChoiceFromOptions(CAMPGROUND_MENU_OPTIONS);
			...
			try { // <------- catches an invalid form of date input and returns search a reservation menu
			...
			System.out
					.println("Site Number:     Max Occupancy:    Accessible?    Max RV Length:    Utilities?   Cost:");
			...
			if(userName.isEmpty()) {
				System.out.println("Name cannot be empty! Returning reservation search menu...");
				displayReservationMenu();
			}
			else {
				Long siteId = sDao.getSiteIdBySiteNumber(siteInput);
				rDao.createReservation(siteId, userName, userArrivalAsLocalDate, userDepartureAsLocalDate);				//rDao.displayNewReservation();
				System.out.println("The reservation has been made and "
						+ "the confirmation id is " + rDao.getMostRecentReservationId());
